...
import { EventsEmit, EventsOff, EventsOn, WindowReload } from '../api/api'
...
function getStockList(value) {
  ...
  // 如果输入的是股票代码格式（包含sh/sz/bj前缀），直接使用
  if (value && (value.startsWith("sh") || value.startsWith("sz") || value.startsWith("bj") || value.startsWith("hk"))) {
...
function onSelect(item) {
  ...
  // item 可能是 "贵州茅台 - sh600519" 格式或直接是 "sh600519"
  if (item.indexOf("-") > 0) {
    // 格式: "贵州茅台 - sh600519"
    data.code = item.split("-")[1].trim().toLowerCase()
  } else if (item.indexOf(".") > 0) {
    // 格式: "600519.SH"
    data.code = item.split(".")[1].toLowerCase() + item.split(".")[0]
  } else {
    // 格式: "sh600519"
    data.code = item.toLowerCase()
  }
  
  // 同时更新 data.name 用于显示
  if (item.indexOf("-") > 0) {
    data.name = item.split("-")[0].trim()
  }
  
  console.log("设置股票代码:", data.code)
}
