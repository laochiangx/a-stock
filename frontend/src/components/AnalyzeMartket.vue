...
import { GlobalStockIndexes, AnalyzeSentimentWithFreqWeight } from "../api/api";
...
function getIndex() {
  GlobalStockIndexes().then((res) => {
    ...
    // Filter main indexes based on name patterns instead of location
    mainIndex.value = (asia.value || []).filter(function (item) {
      return ['上海',"深圳","香港","台湾","北京","东京","首尔","纽约","纳斯达克"].some(loc => item.name.includes(loc))
    }).concat((america.value || []).filter(function (item) {
      return ['上海',"深圳","香港","台湾","北京","东京","首尔","纽约","纳斯达克"].some(loc => item.name.includes(loc))
    }))
    
    chinaIndex.value = (asia.value || []).filter(function (item) {
      return ['上海',"深圳","香港","台湾","北京"].some(loc => item.name.includes(loc))
    })
...
function  handleChart(){
  ...
  AnalyzeSentimentWithFreqWeight("").then((res) => {
    // Check if res object has the expected structure
    if (!res || !res.frequencies) {
      console.error('Invalid response structure:', res);
      return;
    }
    // Check if chart elements are available before initializing
    if (!chartRef.value || !gaugeChartRef.value) {
      console.error('Chart elements not available');
      return;
    }
    let treemapchart, gaugeChart;
    try {
      treemapchart = echarts.init(chartRef.value);
