...
@Component
public class DcHotListSchemaInitializer implements ApplicationRunner {

    private static final Logger log = LoggerFactory.getLogger(DcHotListSchemaInitializer.class);

    private static final int QUERY_TIMEOUT_SECONDS = 3;

    private final JdbcTemplate jdbcTemplate;

    public DcHotListSchemaInitializer(JdbcTemplate jdbcTemplate) {
    ...
    public void run(ApplicationArguments args) {
        try {
            ...
            String tableFqn = schema + ".dc_hot_list_day";

            if (tableExists(tableFqn)) {
                return;
            }

            log.warn("Table dc_hot_list_day does not exist, creating it automatically...");
            ...
            execIgnoreError("CREATE INDEX IF NOT EXISTS idx_dc_hot_list_day_date ON " + tableFqn + " (data_date)");
            execIgnoreError("CREATE INDEX IF NOT EXISTS idx_dc_hot_list_day_order ON " + tableFqn + " (data_date, order_num)");

            log.info("Table {} created/verified.", tableFqn);
        } catch (Exception e) {
            log.error("Failed to ensure dc_hot_list_day schema", e);
        }
    }
